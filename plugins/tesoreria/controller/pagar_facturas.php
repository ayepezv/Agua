      ...
      if($this->empresa->contintegrada)
      {
            ...
            foreach($recibos as $recibo)
            {
               if($recibo->estado != 'Pagado')
               {
                  if( is_null($coddivisa) OR $recibo->coddivisa == $coddivisa )
                  {
                     ...
                     $nombre_cliente = $recibo->nombrecliente;
                  }
                  else
                  {
         ...
         if(!$error)
         {
            ...
            $asientop = $ref0->nuevo_asiento_pago_cli(
                    ...
                    $coddivisa,
                    $tasaconv,
                    'Cobro de facturas de '.$nombre_cliente,
                    $this->cliente,
            ...
            if($asientop)
            {
			   ...
			   $this->new_message('<a target="_new" href="plugins/JRSA/factura_electronica2.php?asiento='.$factura->idasientop.'&codcliente='.$codigo_cliente.'">Generar Factura Electrónica</a>.');               
			   $sql3 = "SELECT importe FROM co_asientos WHERE idasiento =".$factura->idasientop;			   			   
               $neto = $this->db->select_limit($sql3,1,0);				
				$base_imponible=current($neto[0]);
