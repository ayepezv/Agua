            ...
            if ($regularizacion->get_fecha_inside($factura->fecha)) {
                ...
                if ($continuar) {
                    ...
                    if (abs(floatval($_POST['atotal']) - $factura->total) >= .02) {
                        ...
                        $this->new_message("<a href='" . $factura->url() . "'>Factura</a> guardada correctamente.".$factura->codigo."-".$factura->idfactura);
                        $this->new_change('Factura Clientes ' . $factura->codigo, $factura->url(), TRUE);
                        ////////AQUI PONER PARA QUE SE GUARDE EN RECIBO LA FACTURA PARA PODER COBRAR
