...
class ventas_tarifas extends fs_controller
{
   ...
   protected function private_core()
   {
      ...
      $this->tab_articulos = FALSE;
      if( isset($_REQUEST['tab']) )
      {
         if($_REQUEST['tab'] == 'familia')
         {
            $this->template = 'tab_familia';
         }
         else
         {
            $this->tab_articulos = TRUE;
         }
      }
   ...
   private function share_extensions()
   {
      ...
      $fsext = new fs_extension();
      $fsext->name = 'tab_articulos';
      $fsext->from = __CLASS__;
      $fsext->to = 'ventas_articulos';
      $fsext->type = 'tab';
      $fsext->text = 'Tarifas';
      $fsext->params = '&tab=articulos';
      $fsext->save();
      
      unset($fsext);
      
      $fsext = new fs_extension();
      $fsext->name = 'tab_familia';
      $fsext->from = __CLASS__;
      $fsext->to = 'ventas_familia';
      $fsext->type = 'tab';
      $fsext->text = 'Tarifas';
      $fsext->params = '&tab=familia';
      $fsext->save();
   }
   ...
   {
      if( $this->db->table_exists('articulostarifas') )
      {
         $this->db->exec("DELETE FROM articulostarifas WHERE codtarifa NOT IN (SELECT codtarifa FROM tarifasav);");
      }
   }
}
